...
현재 체크아웃의 동작을 유지하는 리팩터링이다. 기존 사용자 변경인 Polymarket 게이트, 로그인할 때 저장 주소 초기화, 마켓/리더보드 전환을 보존했다. 이전 요청에 등장한 기능 변경을 다시 적용하지 않았다.

- Hub: 소개 문구 → PERP-DEX DAY → Wonderland → TOKEN2049, 현재의 **5초** 주기 유지. 키보드 포커스·화면 밖·모션 감소 설정에 따른 정지도 유지.
- 공개 행사 소개, Google 로그인 단계, TOKEN2049 Polymarket 단계, 모달 취소와 복귀 경로 유지.
...
│   ├── PolymarketAccountModal.tsx     # Polymarket 모달 UI
│   ├── polymarketAccount.ts           # 기존 데모 주소 게이트
│   └── auth.css                   # 인증 UI 스타일
├── infrastructure/
...
4. `useMarketAccess`는 Google SDK와 저장소를 import하지 않는다. 성공 후 Polymarket 주소 초기화는 조립 지점인 `App`이 실행한다.
...
완전한 프레임워크 독립 도메인 계층으로 전면 재작성한 것은 아니다. 기존 시장 React 훅에는 저장 및 preview 어댑터 의존성이 있고, `polymarketAccount`도 데모 저장소를 직접 사용한다. 이 경계를 보존함으로써 기능 변경을 피했다.
...
Google/Polymarket는 기존 **데모 동작**을 유지했다. Client ID가 없으면 모달 버튼이 게이트를 열고, 검증 API가 없으면 서버 검증을 생략한다. Polymarket 주소 저장도 실제 계정 검증이 아니다. 이 리팩터링은 보안 인증을 완성하지 않으며 실제 운영에는 별도의 서버 검증이 필요하다.
