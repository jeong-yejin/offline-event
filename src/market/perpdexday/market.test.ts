...
import { INITIAL_TRADER_BALANCE, SETTLEMENT_PRICE, STARTING_POINT } from '../../data/perpDexMarketContent';
import { PERP_DEX_IDS, DEMAND_MAX, MOMENTUM_MAX, OPEN_BALANCES, OPEN_INTEREST, OPEN_PRICES, balanceComponent, demandAdjustment, demandImbalance, midOf, midPrices, momentumAdjustment, momentumOf, normalize, quote, rfqPrice, settlementPayout, settlementPrice, winnersOf, type BalanceMap, type Holding, type OpenMap, type PriceMap } from './market';
import { createQuote, parseQuantity, type OrderQuote } from '../quote';
import { reduceMarket, type MarketState } from './state';

const NOW = 1757000000000;

const bookTotal = (prices: PriceMap) => PERP_DEX_IDS.reduce((total, id) => total + prices[id], 0);

const balancesOf = (values: readonly number[]): BalanceMap =>
  PERP_DEX_IDS.reduce((map, id, index) => ({ ...map, [id]: values[index] }), {} as BalanceMap);
...
const stateWith = (overrides: Partial<MarketState> = {}): MarketState => ({
  ...
  prices: OPEN_PRICES,
  open: OPEN_INTEREST,
  updatedAt: PERP_DEX_IDS.reduce((map, id) => ({ ...map, [id]: NOW }), {} as Record<string, number>) as MarketState['updatedAt'],
  history: [],
  fills: [],
  volume: 0,
...
describe('market price pipeline', () => {
  ...
  it('opens every trader level, because they all start on the same margin balance', () => {
    PERP_DEX_IDS.forEach((id) => expect(OPEN_PRICES[id]).toBeCloseTo(25, 6));
  });
...
describe('settlement', () => {
  ...
  it('holds the market in settling before it names a winner, because final balances are confirmed first', () => {
  ...
  it('records the settled amount without crediting point, because the prize is handed out days after the event', () => {
    const open = stateWith({ point: 0, holdings: [{ id: 'variational', side: 'yes', qty: 3, cost: 78 }], balances: balancesOf([14200, 13800, 11500, 9700]) });
    const settling = reduceMarket(open, { type: 'tick', time: open.closeAt, seed: 1 });
    const ended = reduceMarket(settling, { type: 'tick', time: open.settleAt, seed: 1 });

    expect(ended.settlement?.payout).toBe(3 * SETTLEMENT_PRICE);
    expect(ended.point).toBe(0);
  });
